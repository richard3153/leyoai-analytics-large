...
# leyoai-analytics-large

## 中文介绍

LeyoAI Analytics Model (Large) 是基于 Qwen2.5-7B-Instruct 微调的数据分析助手，提供最强的大数据分析能力。
...
model = AutoModelForCausalLM.from_pretrained(
    "FFZwai/leyoai-analytics-large",
    device_map="auto"
)
tokenizer = AutoTokenizer.from_pretrained("FFZwai/leyoai-analytics-large")

# Inference
prompt = "设计 A/B 测试方案并分析统计显著性"
...
LeyoAI Analytics Model (Large) is a data analytics assistant fine-tuned from Qwen2.5-7B-Instruct, providing the strongest big data analysis capabilities.
...
# Inference
prompt = "Design an A/B testing plan and analyze statistical significance"
inputs = tokenizer(prompt, return_tensors="pt").to(model.device)
outputs = model.generate(**inputs, max_length=100)
response = tokenizer.decode(outputs[0], skip_special_tokens=True)
print(response)
